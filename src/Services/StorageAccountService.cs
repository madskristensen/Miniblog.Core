...
{
   public class StorageAccountService : IBlogService
   {
      ...
      public StorageAccountService(IConfiguration configuration, IHttpContextAccessor contextAccessor)
      {
         ...
         this.connString = configuration["AzureStorage:ConnectionString"];
      ...
      {
         await this.LoadPosts().ConfigureAwait(false);
         this.SortCache();
      }

      private CloudBlobContainer GetContainerClient(string containerName)
      {
         CloudStorageAccount storageAccount;
         if (CloudStorageAccount.TryParse(this.connString, out storageAccount))
         {
         ...
         else
         {
            throw new Exception("A connection string has not been defined in the system environment variables. " +
                "Add an environment variable named 'AzureStorage:ConnectionString' with your storage " +
      ...
      {
         var containerClient = GetContainerClient(POSTS);
         List<Uri> blobs = new List<Uri>();
         BlobContinuationToken? blobContinuationToken = null;
         do
         {
            var results = await containerClient.ListBlobsSegmentedAsync(null, blobContinuationToken).ConfigureAwait(false);
         ...
         {
            CloudBlockBlob blobClient = containerClient.GetBlockBlobReference(Path.GetFileName(file.LocalPath));
            var download = await blobClient.DownloadTextAsync().ConfigureAwait(false);

            var doc = XElement.Parse(download);

            var post = new Post
            {
               ID = Path.GetFileNameWithoutExtension(file.LocalPath),
      ...
      {
         var containerClient = GetContainerClient(POSTS);
         CloudBlockBlob blobClient = containerClient.GetBlockBlobReference(post.ID);
      ...
      public async Task<string> SaveFile(byte[] bytes, string fileName, string? suffix = null)
      {
         ...
         var containerClient = GetContainerClient(FILES);
         CloudBlockBlob blobClient = containerClient.GetBlockBlobReference(fileNameWithSuffix);
         await blobClient.UploadFromByteArrayAsync(bytes, 0, bytes.Length).ConfigureAwait(false);
      ...
      public async Task SavePost(Post post)
      {
         ...
         var containerClient = GetContainerClient(POSTS);
         CloudBlockBlob blobClient = containerClient.GetBlockBlobReference(post.ID);
         await blobClient.UploadTextAsync(doc.ToString()).ConfigureAwait(false);
